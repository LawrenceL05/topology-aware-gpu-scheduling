...
This collector covers intra-node GPU relationships between GPUs. The host-side
view, which NUMA node owns a GPU and which interface is nearest to it, is
collected separately; see
[GPU, NUMA, and NIC host locality](host-topology.md). Full NIC inventory, a
typed affinity graph, and inter-node bandwidth and latency still require
separate collectors and measurements.
